...
export async function queryGemini(businessName, query) {
  ...
  // For each top candidate, try with grounding (both tool variants) then without.
  const toolVariants = [
    [{ googleSearch: {} }],
  ...
  return {
    raw: '',
    error: `Gemini failed across ${candidates.slice(0, 3).join(', ')}: ${lastError?.message || 'unknown'}`,
  };
}
