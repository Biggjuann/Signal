...
function buildPrompt(_businessName, query) {
  // The prompt must NOT mention the business by name (biases toward false
  // positives) and must NOT instruct the model to enumerate brand names
  // ("name actual businesses, not generic categories" was over-aggressive).
  // But asking for a list is fair — that's how consumer ChatGPT and Gemini
  // naturally respond to "best X near Y" queries. A bare query produces
  // conversational responses that under-enumerate, causing false negatives.
  return `${query}\n\nList the top recommendations with a brief description of each.`;
}
...
export async function queryGemini(businessName, query) {
  ...
  // System instruction nudges the API toward consumer-app-like behavior:
  // a tight top-N list grounded in current sources only.
  const systemInstruction =
    'You are answering as the Gemini consumer assistant would. For local ' +
    'recommendations, base your answer ONLY on current web search results — ' +
    'never on training-data recall. Limit to the top 5–7 most relevant, ' +
    'currently-operating businesses. If a business does not appear in current ' +
    'search results, do not mention it.';
  ...
  for (const modelName of candidates.slice(0, 3)) {
    for (const tools of toolVariants) {
      try {
        ...
        // Extract the actual web sources Gemini grounded on. The brand must
        // appear in these — otherwise the response is padding from training
        // data, not a real signal of current visibility.
